  ...
  function checkboxes(marcadas) {
    ...
    return Object.keys(grupos).map(g => `<div style="margin-bottom:var(--s3)">
      ...
      <div class="perm-grid">${grupos[g].map(p => `
        <label class="perm-item"><input type="checkbox" data-perm="${p.id}"${marcadas.includes(p.id) ? ' checked' : ''}>
        <span>${Fmt.esc(p.nome)}</span></label>`).join('')}</div></div>`).join('');
  }
  ...
  function editar(id) {
    ...
    UI.modal(orig ? 'Editar usuário' : 'Cadastrar usuário', `
      ...
      <div style="margin-top:var(--s4)">
        ...
        <div id="uu-perms">${checkboxes(u.permissoes || [])}</div>
    ...
    $('uu-perfil').addEventListener('change', () => {
      descPerfil();
      $('uu-perms').innerHTML = checkboxes(Perm.doPerfil($('uu-perfil').value));
    });
    $('uu-padrao').addEventListener('click', () => { $('uu-perms').innerHTML = checkboxes(Perm.doPerfil($('uu-perfil').value)); });
